...
-- 缓存
create table auth_user_jwt_cache {
	user_id int not null,
	jwt_version int not null
	primary key(user_id)
}
comment on table auth_user_jwt_cache is 'jwt缓存,放redis';
comment on column auth_user_jwt_cache.jwt_version is '修改密码、作废、修改角色等，需要换token';

create table auth_version_cache {
	auth_key varchar(32) not null,
	auth_version int not null,
	primary key(auth_key)
}
comment on table auth_version_cache is '权限版本,放redis';
comment on column auth_version_cache.auth_version is '修改权限时，需要刷新内存';
